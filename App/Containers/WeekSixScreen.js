...
//move this
  ...
  selectContestant(contestant){
    ...
    if(contestant.added){
      ...
      this.setState({weekSixPicks: this.state.weekSixPicks.concat([contestant.title])});
    }
    else{
      for(let i =0; i<this.state.weekSixPicks.length; i++){
        if(this.state.weekSixPicks[i] === contestant.title){
          this.setState({
            weekSixPicks: update(this.state.weekSixPicks, {$splice: [[i, 1]]})
          })
        }
      }
    }
  ...
  attemptNextScreen () {
    if(this.state.weekSixPicks.length >= 4){
      //write to databse
      let currbracket = '-KQafDdpleLD1sxKH2wd'
      let uid  = Firebase.auth().currentUser.v
      firebase.database().ref('brackets/' + currbracket + `/${uid}`).child('weekSixPicks').set({
        0 : this.state.weekSixPicks[0],
        1 : this.state.weekSixPicks[1],
        2 : this.state.weekSixPicks[2],
        3 : this.state.weekSixPicks[3],
      })

      this.props.weekSeven();
    }
    else{
      //alert here
  ...
  render () {
    console.log('this.state.weekSixPicks:', this.state.weekSixPicks)
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text  style={styles.subtitle}>Select 15... {this.state.weekSixPicks.length}/15</Text>
        </View>
        ...
        <RoundedButton text='Week 7 ->' onPress={this.attemptNextScreen} />
      </View>
    )
  }
}
...
const mapStateToProps = (state) => {
  return {
   ...
   // weekSixPicks: NavigationActions.weekSeven
