...
class WeekEightScreen extends React.Component {

  static propTypes = {
    bracket: PropTypes.func,
    weekEightPicks: PropTypes.array,
...
//move this
  ...
  selectContestant(contestant){
    ...
    if(contestant.added){
      ...
      this.setState({weekEightPicks: this.state.weekEightPicks.concat([contestant.title])});
    }
    else{
      for(let i =0; i<this.state.weekEightPicks.length; i++){
        if(this.state.weekEightPicks[i] === contestant.title){
          this.setState({
            weekEightPicks: update(this.state.weekEightPicks, {$splice: [[i, 1]]})
          })
        }
      }
    }
  ...
  attemptNextScreen () {
    if(this.state.weekEightPicks.length >= 4){
      //write to databse
      let currbracket = '-KQafDdpleLD1sxKH2wd'
      let uid  = Firebase.auth().currentUser.v
      firebase.database().ref('brackets/' + currbracket + `/${uid}`).child('weekEightPicks').set({
        0 : this.state.weekEightPicks[0],
        1 : this.state.weekEightPicks[1],
        2 : this.state.weekEightPicks[2],
        3 : this.state.weekEightPicks[3],
      })
  ...
  render () {
    console.log('this.state.weekEightPicks:', this.state.weekEightPicks)
    console.log('this in render', this)

    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.title}>Week 8</Text>
          <Text  style={styles.subtitle}>Select 15... {this.state.weekEightPicks.length}/15</Text>
        </View>
        ...
        <RoundedButton text='Done' onPress={this.attemptNextScreen} />
      </View>
    )
  }
}
...
const mapStateToProps = (state) => {
  return {
   ...
   // weekEightPicks: NavigationActions.weekSeven
...
export default connect(mapStateToProps)(WeekEightScreen)
