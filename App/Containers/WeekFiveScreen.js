...
class WeekFiveScreen extends React.Component {

  static propTypes = {
    weekSix: PropTypes.func,
    weekFivePicks: PropTypes.array,
...
//move this
  ...
  selectContestant(contestant){
    ...
    if(contestant.added){
      ...
      this.setState({weekFivePicks: this.state.weekFivePicks.concat([contestant.title])});
    }
    else{
      for(let i =0; i<this.state.weekFivePicks.length; i++){
        if(this.state.weekFivePicks[i] === contestant.title){
          this.setState({
            weekFivePicks: update(this.state.weekFivePicks, {$splice: [[i, 1]]})
          })
        }
      }
    }
  ...
  attemptNextScreen () {
    if(this.state.weekFivePicks.length >= 4){
      //write to databse
      let currbracket = '-KQafDdpleLD1sxKH2wd'
      let uid  = Firebase.auth().currentUser.v
      firebase.database().ref('brackets/' + currbracket + `/${uid}`).child('weekFivePicks').set({
        0 : this.state.weekFivePicks[0],
        1 : this.state.weekFivePicks[1],
        2 : this.state.weekFivePicks[2],
        3 : this.state.weekFivePicks[3],
      })
  ...
  render () {
    console.log('this.state.weekfivepicks:', this.state.weekFivePicks)
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text  style={styles.subtitle}>Select 15... {this.state.weekFivePicks.length}/15</Text>
        </View>
        ...
        <RoundedButton text='Week 6 ->' onPress={this.attemptNextScreen} />
      </View>
    )
  }
}
...
const mapStateToProps = (state) => {
  return {
    weekSix: NavigationActions.weekSix
   // weekFivePicks: NavigationActions.weekSix
...
export default connect(mapStateToProps)(WeekFiveScreen)
