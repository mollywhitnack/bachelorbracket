...
class WeekSevenScreen extends React.Component {

  static propTypes = {
    weekEight: PropTypes.func,
    weekSevenPicks: PropTypes.array,
...
//move this
  ...
  selectContestant(contestant){
    ...
    if(contestant.added){
      ...
      this.setState({weekSevenPicks: this.state.weekSevenPicks.concat([contestant.title])});
    }
    else{
      for(let i =0; i<this.state.weekSevenPicks.length; i++){
        if(this.state.weekSevenPicks[i] === contestant.title){
          this.setState({
            weekSevenPicks: update(this.state.weekSevenPicks, {$splice: [[i, 1]]})
          })
        }
      }
    }
  ...
  attemptNextScreen () {
    if(this.state.weekSevenPicks.length >= 4){
      //write to databse
      let currbracket = '-KQafDdpleLD1sxKH2wd'
      let uid  = Firebase.auth().currentUser.v
      firebase.database().ref('brackets/' + currbracket + `/${uid}`).child('weekSevenPicks').set({
        0 : this.state.weekSevenPicks[0],
        1 : this.state.weekSevenPicks[1],
        2 : this.state.weekSevenPicks[2],
        3 : this.state.weekSevenPicks[3],
      })

      this.props.weekEight();
    }
    else{
      //alert here
  ...
  render () {
    console.log('this.state.weekSevenPicks:', this.state.weekSevenPicks)
    console.log('this in render', this)

    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.title}>Week 7</Text>
          <Text  style={styles.subtitle}>Select 15... {this.state.weekSevenPicks.length}/15</Text>
        </View>
        ...
        <RoundedButton text='Week 8 ->' onPress={this.attemptNextScreen} />
      </View>
    )
  }
}
...
const mapStateToProps = (state) => {
  return {
    weekEight: NavigationActions.weekEight
   // weekSevenPicks: NavigationActions.weekSeven
...
export default connect(mapStateToProps)(WeekSevenScreen)
